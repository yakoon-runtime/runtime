...
def parse_entry(entry: str) -> tuple[str, str]:
    ...
    Schemes:
      cap:<module>:<function>  — Python package entry point
      file:<path>               — File-based entry point
    """
    if ":" in entry:
        scheme, _, rest = entry.partition(":")
        if scheme in ("cap", "file"):
            return (scheme, rest)
    msg = f"invalid entry '{entry}' — expected 'cap:<module>:<func>' or 'file:<path>'"
    ...
    for i, line in enumerate(output.splitlines()):
        ...
        if line.startswith("{"):
            try:
                ...
                pass
        pulses.append(out_text(line, mode=mode))
    return pulses
