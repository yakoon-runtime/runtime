"""Cap reference linking (ADR-10).

The runtime resolves ``cap:<module>:<func>`` for exactly one purpose:
...
class CapReference:
    """A ``cap:<module>:<func>`` bootstrap reference.
    ...
    def __init__(self, expr: str):
        ...
        if scheme != "cap":
            raise ValueError(f"invalid cap reference: {expr!r}")
