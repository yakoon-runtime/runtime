...
from y5n.runtime.engine.executor.cap import CapReference
        ...
        for path_key, node in sorted(self._nodes.items()):
            ...
            for phase in ("run", "setup"):
                ...
                if not isinstance(entry_path, str):
                    continue
                if entry_path.startswith("cap:") or entry_path.startswith("file:"):
                    continue
                issues.append(
                    f"{phase}: invalid entry '{entry_path}'"
                    f" — expected 'cap:...' or 'file:...'"
                )
...
def _make_resolve_handler(resolve_expr: str):
    """Build a node's resolve handler from a ``cap:<module>:<func>`` expression.
